...
fetchAccount = async function(username) {
    const data = {
        id: 3,
        jsonrpc: "2.0",
        method: "call",
...
fetchFollowCount = async function(username) {
    const data = {
        id: 4,
        jsonrpc: "2.0",
        method: "call",
    ...
    return (
        <Container style={{ flex:1, background: 'white'}}>
            <Header style={{backgroundColor: 'white'}}>
                <Left><Icon name="md-person-add" size={22} style={{ paddingLeft:10 }} /></Left>
                <Body><Text style={{fontSize: 16, fontWeight:'600'}}>{name}</Text></Body>
                <Right><EntypoIcon name="back-in-time" size={22} style={{ paddingRight:10, fontSize: 32 }} /></Right>
            </Header>
            <Content>
